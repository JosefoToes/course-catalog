...
function showall() {
    for (i = courses.length - 1; i >= 0; i--) {
        if (courses[i].partner_id != '') {
            link = '\'' + domain2 + '/' + courses[i].internal_id + '\'';
            audience = 'partner';
        } else {
            link = '\'' + domain + '/' + courses[i].partner_id + '\'';
        }
        ...
        document.getElementById('courses_list').innerHTML += '<div class="col-10 col-sm-5 col-md-3 valueslist ' + audience + ' ' + ltype + '" id="v' + i + '"><span class="category" style="display:' + confdnt + ';">Private</span><figure><a href=' + link + ' target="_blank"><img src="img/' + i + '.jpg" alt="Hero image"></a><figcaption><a href=' + link + ' target="_blank"><h3>' + courses[i].title + '</h3></a></figcaption></figure></div>';
        newkeywords = courses[i].title.split(' ');
        courses[i].keywords = newkeywords.concat(courses[i].info);
        courses[i].keywords.push(month, year, month + ', ' + year, audience);
        courses[i].keywords.forEach(function(keyword) {
            if (keyword == 'public') {
                courses[i].keywords.push('public courses');
            }
            if (keyword == 'private') {
                courses[i].keywords.push('private courses');
            }
        });
    }
